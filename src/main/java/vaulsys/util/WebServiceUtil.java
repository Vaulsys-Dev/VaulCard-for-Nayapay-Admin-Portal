...
//        }
        ...
        if(Util.hasText(wsmodel.getCardnolastdigits()))
        {
            logger.info("CardNoLastDigits [" + wsmodel.getCardnolastdigits() + "]");
        }
...
//    }
    ...
    //Huzaifa Added - 19072024 - VP-NAP-202307181 ==> De-Link All Linked Bank Accounts on Reset MPIN State
    public static String getRSAEncryptedValue(String value) {
        String encryptedValue = "", rsakey = "";
        try {
            rsakey = GlobalContext.getInstance().getPanEncryptionKeys().get(KeyType.TYPE_RSA_PUBLIC);
            if (!Util.hasText(rsakey)) {
                logger.error("RSA or AES key not found!!!");

            } else {
                //TODO: need to comment below logging
                logger.info("Key [" + rsakey + "]");
                encryptedValue = RSAEncryptByKey(rsakey, value);
            ...
            e.printStackTrace();
            logger.error("Unable to encrypt Value!!!");
            encryptedValue = "";
        }
        return encryptedValue;
    }

    public static String RSAEncryptByKey(String publicKeyStr, String originalText)
        ...
        return Base64.getEncoder().encodeToString(RSA_obj.encrypt(originalText, publicKey));
    }
    //===================================================================================================
