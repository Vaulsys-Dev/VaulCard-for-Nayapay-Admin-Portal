 ...
 */
public class SimpleMainWSProcess {

    private static final Logger logger = Logger.getLogger(SimpleMainWSProcess.class);
    ...
    public SimpleMainWSProcess(long Id, WalletCMSWsEntity wsobj) {
        this.id = Id;
        this.wsobj = wsobj;
    }

    public WalletCMSWsEntity processTransaction() {
        try {
            ...
            switch(servicename)
            {
                ...
                case "GetCardNumber":
                {
                    WebServiceHandler.processGetCardNumber(wsobj);
                    break;
                }
                // ======================================================================================

                // Asim Shahzad, Date : 19th April 2022, Project ID : NAP-2-22
                case "BlockMerchantWallet":
                {
                    WebServiceHandler.processBlockMerchantWallet(wsobj);
                    break;
                }
                // ============================================================

                // Asim Shahzad, Date : 5th Feb 2023, Tracking ID : VP-NAP-202302021 / VC-NAP-202302021
                case "BlockAllDebitCards":
                {
                    WebServiceHandler.processBlockAllDebitCards(wsobj);
                    break;
                }
                // ====================================================================================
                default:
            ...
            logger.error(WebServiceUtil.getStrException(e));//e.printStackTrace();
        ...
        //m.rehman: 10-11-2021 - Nayapay Optimization
            ...
            SwitchTransactionCodes txnCode = GlobalContext.getInstance().getTransactionCodeDescbyCode(wsobj.getServicename());
            if (txnCode != null &&
                    ((txnCode.getIsfinancial() != null && txnCode.getIsfinancial())
                            || (txnCode.getIsBypass() != null && !txnCode.getIsBypass()))) {
                //listing.setId(wsobj.getId());
                GeneralDao.Instance.beginTransaction(GeneralDao.OPTIMIZER_MODE_FIRST_ROWS);
