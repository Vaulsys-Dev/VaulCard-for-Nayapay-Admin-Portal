...
public class WalletCMSWebResource {
	...
	// Asim Shahzad, Date : 19th Jan 2022, Tracking ID : VP-NAP-202201132 / VC-NAP-202201135
	@Path("/getcardnumber")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})  //add MediaType.APPLICATION_XML if you want XML as well (don't forget @XmlRootElement)
	public WalletCMSWsEntity GetCardNumber(WalletCMSWsEntity wsobj) throws Exception{

		try {
			logger.info("WebResource GetCardNumber Request Received");
			if(wsobj == null)
			{
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
			...
			wsobj.setServicename("GetCardNumber");
			//m.rehman: for validating incoming ip address
			wsobj.setIncomingip(request.getRemoteAddr());
			SimpleMainWSProcess process = new SimpleMainWSProcess(WalletCMSWSServer.id++,wsobj);
			return process.processTransaction();
		...
		{
			e.printStackTrace();
			logger.error("Exception caught while executing GetCardNumber call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending GetCardNumber reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		//m.rehman: 15-11-2021, Nayapay Optimization
		finally {
	...
	@Path("/blockmerchantwallet")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})  //add MediaType.APPLICATION_XML if you want XML as well (don't forget @XmlRootElement)
	public WalletCMSWsEntity BlockMerchantWallet(WalletCMSWsEntity wsobj) throws Exception{

		try {
			logger.info("WebResource BlockMerchantWallet Request Received");
			if(wsobj == null)
			{
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
			...
			wsobj.setServicename("BlockMerchantWallet");
			//m.rehman: for validating incoming ip address
			wsobj.setIncomingip(request.getRemoteAddr());
			SimpleMainWSProcess process = new SimpleMainWSProcess(WalletCMSWSServer.id++,wsobj);
			return process.processTransaction();
		...
		{
			e.printStackTrace();
			logger.error("Exception caught while executing BlockMerchantWallet call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending BlockMerchantWallet reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		//m.rehman: 15-11-2021, Nayapay Optimization
		finally {
	...
	@Path("/blockalldebitcards")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public WalletCMSWsEntity BlockAllDebitCards(WalletCMSWsEntity wsobj) throws Exception {
		try {
			logger.info("WebResource BlockAllDebitCards Request Received");
			if(wsobj == null)
			{
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
			...
			wsobj.setServicename("BlockAllDebitCards");
			wsobj.setIncomingip(request.getRemoteAddr());
			SimpleMainWSProcess process = new SimpleMainWSProcess(WalletCMSWSServer.id++,wsobj);
			return process.processTransaction();
		}
		...
		{
			e.printStackTrace();
			logger.error("Exception caught while executing BlockAllDebitCards call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending BlockAllDebitCards reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		finally {
			HttpSession session = request.getSession(false);
