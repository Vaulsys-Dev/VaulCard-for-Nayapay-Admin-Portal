...
public class WalletCMSFunctions {
    ...
    public static boolean CreateAccountProfile(WalletCMSWsEntity wsmodel, CMSAccount account)
    {
        try
        {
            ...
            //new start
            dbQuery = "from " + SystemConfig.class.getName() + " c where c.identifier= :IDENTIFIER";
            params = new HashMap<String, Object>();
            params.put("IDENTIFIER", "SEC_QUES_RETRIES");

            SystemConfig obj_SystemConfig = (SystemConfig)GeneralDao.Instance.findObject(dbQuery, params);
            // ===============================================================
            ...
            if(account == null)
            {
                ...
                // Asim Shahzad, Date : 27th Aug 2020, Call ID : VC-NAP-202008073/ VC-NAP-202009301
                customer.setSecretQuestionRetries(obj_SystemConfig.getValue());
                //===============================================================
        ...
        if(posEntryMode != null) {
            ...
            if (panEntryCapability.equals(ISOPOSEntryMode.PANEntryMode.MST_READ_CVV_POSSIBLE)
                ...
                if (Util.hasText(wsmodel.getChannelid())
                        && wsmodel.getChannelid().equals(ChannelCodes.EURONET)
